...
struct is_std_string<
  ...
  typename types::enable_if<
    ...
    && types::is_same<decltype(declval<const T&>().size()), size_t>::value>::
    type> : types::true_type {};
...
public:
  ...
  template<
    typename T,
    typename detail::types::enable_if<detail::is_std_string<T>::value, bool>::
      type = true>
  ...
  friend DOCTEST_INTERFACE std::ostream& operator<<(
    std::ostream& s,
    const String& in);
};
...
template<
  ...
  typename detail::types::
    enable_if<detail::should_stringify_as_underlying_type<T>::value, bool>::type
  = true>
String toString(const DOCTEST_REF_WRAP(T) value)
{
...
struct filldata<const volatile void*> {
  DOCTEST_INTERFACE static void fill(
    std::ostream*        stream,
...
struct DOCTEST_INTERFACE Approx {
  ...
  explicit Approx(
    const T& value,
    typename detail::types::enable_if<std::is_constructible<double, T>::value>::
      type* = static_cast<T*>(nullptr))
  {
...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
  template<typename T>
  typename std::enable_if<std::is_constructible<double, T>::value, Approx&>::
    type epsilon(const T& newEpsilon)
  {
...
#define DOCTEST_APPROX_PREFIX \
  template<typename T>        \
  friend typename std::       \
    enable_if<std::is_constructible<double, T>::value, bool>::type
...
#define DOCTEST_SUBCASE(name)                                                \
  if (                                                                       \
    const doctest::detail::Subcase& DOCTEST_ANONYMOUS(DOCTEST_ANON_SUBCASE_) \
...
#define DOCTEST_INFO(...)                      \
  DOCTEST_INFO_IMPL(                           \
    DOCTEST_ANONYMOUS(DOCTEST_CAPTURE_MB_),    \
...
{
#if DOCTEST_MSVC
  if (m_expr[0] == ' ') // this happens when variadic macros are disabled under
                        // MSVC
    ++m_expr;
#endif // MSVC
}
...
bool ResultBuilder::log()
{
  if (m_at & assertType::is_throws) {
    ...
    (m_at & assertType::is_throws_as) && (m_at & assertType::is_throws_with)) {
...
struct ConsoleReporter : public IReporter {
  ...
  static const char* getSuccessOrFailString(
    bool             success,
    assertType::Enum at,
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)                  \
  ...
  DOCTEST_PARSE_AS_BOOL_OR_FLAG(
    "gnu-file-line", "gfl", gnu_file_line, !bool(DOCTEST_MSVC));
  DOCTEST_PARSE_AS_BOOL_OR_FLAG(
    "no-path-filenames", "npf", no_path_in_filenames, false);
  DOCTEST_PARSE_STR_OPTION(
    "strip-file-prefixes", "sfp", strip_file_prefixes, "");
  DOCTEST_PARSE_AS_BOOL_OR_FLAG(
    "no-line-numbers", "nln", no_line_numbers, false);
  DOCTEST_PARSE_AS_BOOL_OR_FLAG(
    "no-debug-output", "ndo", no_debug_output, false);
  DOCTEST_PARSE_AS_BOOL_OR_FLAG(
    "no-skipped-summary", "nss", no_skipped_summary, false);
  DOCTEST_PARSE_AS_BOOL_OR_FLAG(
    "no-time-in-output", "ntio", no_time_in_output, false);
...
bool checkIfShouldThrow(assertType::Enum at)
{
  ...
  if (
    (at & assertType::is_check) && getContextOptions()->abort_after > 0
...
const char* ConsoleReporter::getSuccessOrFailString(
  bool             success,
  assertType::Enum at,
  const char*      success_str)
{
...
Color::Enum ConsoleReporter::getSuccessOrFailColor(
  bool             success,
  assertType::Enum at)
{
...
// this was requested to be made virtual so users could override it
void ConsoleReporter::file_line_to_stream(
  const char* file,
  int         line,
  const char* tail)
{
...
void ConsoleReporter::printRegisteredReporters()
{
  printVersion();
  auto printReporters
    = [this](const detail::reporterMap& reporters, const char* type) {
        if (!reporters.empty()) {
          s << Color::Cyan << "[doctest] " << Color::None
...
void ConsoleReporter::test_run_end(const TestRunStats& p)
{
  ...
  auto       totwidth  = static_cast<int>(std::ceil(log10(
    static_cast<double>(std::max(
      p.numTestCasesPassingFilters, static_cast<unsigned>(p.numAsserts)))
    + 1)));
...
void JUnitReporter::JUnitTestCaseData::add(
...
void JUnitReporter::JUnitTestCaseData::addError(
  const std::string& message,
  const std::string& details)
{
...
{
  if (isOnStack())
    return last - (static_cast<size_type>(buf[last]) & 31); // using "last"
                                                            // would work only
                                                            // if "len" is 32
  return data.size;
}
...
void filldata<const volatile void*>::fill(
  std::ostream*        stream,
  const volatile void* in)
{
  if (in) {
...
{
  if (
    g_cs->traversal.activeSubcaseDepth()
