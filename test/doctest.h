...
#ifdef DOCTEST_CONFIG_INCLUDE_TYPE_TRAITS
  template<typename T>
  typename detail::enable_if<std::is_constructible<double, T>::value, Approx&>::
    type
...
#define DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(op, op_str, op_macro)      \
  ...
  {                                                                        \
    bool res = op_macro(                                                   \
      doctest::detail::forward<L>(lhs), doctest::detail::forward<R>(rhs)); \
...
{
  if (isOnStack())
    return last - (unsigned(buf[last]) & 31); // using "last" would work only if
                                              // "len" is 32
  return data.size;
}
DOCTEST_GCC_SUPPRESS_WARNING_POP
  ...
  : m_signature({name, file, line})
{
  ...
  if (s->subcasesStack.size() < size_t(s->subcase_filter_levels)) {
    if (!matchesAny(
      ...
      return;
    if (matchesAny(
          m_signature.m_name.c_str(), s->filters[7], false, s->case_sensitive))
      return;
  }
    ...
    static bool       execute = true;
    {
      ...
      if (execute) {
        ...
        for (size_t i = 0; i < DOCTEST_COUNTOF(signalDefs); ++i) {
          if (
            ExceptionInfo->ExceptionRecord->ExceptionCode == signalDefs[i].id) {
  ...
  String*            value)
{
  ...
  for (int i = argc; i > 0; --i) {
    ...
    if (temp && (value || strlen(temp) == strlen(pattern))) { //! OCLINT prefer
                                                              //! early exits
                                                              //! and continue
