...
enum Enum {
    ...
    = is_normal | is_fast | is_false | is_unary | is_warn,
    DT_FAST_CHECK_UNARY_FALSE
    = is_normal | is_fast | is_false | is_unary | is_check,
    DT_FAST_REQUIRE_UNARY_FALSE
    = is_normal | is_fast | is_false | is_unary | is_require
};
} // namespace assertType
...
public:
    ...
    template<typename T>
    typename detail::
        enable_if<std::is_constructible<double, T>::value, Approx&>::type
    ...
    do {                                                                       \
        if (!is_running_in_test) {                                             \
            if (failed) {                                                      \
                ...
                int res                                                        \
                    = checkIfShouldThrow(at) ? assertAction::shouldthrow : 0;  \
...
template<typename L>
DOCTEST_NOINLINE DOCTEST_FAST_ASSERT_RETURN_TYPE fast_unary_assert(
    assertType::Enum at,
    const char*      file,
    int              line,
    const char*      expr,
...
struct DOCTEST_INTERFACE IReporter {
    ...
    // have to be traversed - check CurrentTestCaseStats::should_reenter
    // (caching a pointer to the input doesn't make sense here)
    ...
    struct DOCTEST_CAT(id, ITERATOR) {                           \
        ...
        {                                                        \
            DOCTEST_REGISTER_TYPED_TEST_CASE_IMPL(               \
                anon<Type>, Type, dec, line * 1000 + index);     \
...
thread_local bool
    g_no_colors; // used to avoid locks for the debug output window reporter
    ...
    : m_signature(name, file, line)
{
    ...
    if (s->subcasesCurrentLevel < s->subcase_filter_levels) {
        if (!matchesAny(
                m_signature.m_name, s->filters[6], true, s->case_sensitive))
            return;
        if (matchesAny(
                m_signature.m_name, s->filters[7], false, s->case_sensitive))
            return;
    }
...
thread_local std::vector<IContextScope*>
...
struct ConsoleReporter : public IReporter {
    ...
    void test_case_end(const CurrentTestCaseStats& st) override
    {
        ...
        // means the test case will be re-entered because there are untraversed
        // (but discovered) subcases
    ...
    void log_assert(const AssertData& rb) override
    {
        ...
        if (rb.m_at & assertType::is_throws) { //! OCLINT bitwise operator in
                                               //! conditional
            s << (rb.m_threw ? "threw as expected!" : "did NOT throw at all!")
              << "\n";
        }
        else if (rb.m_at & assertType::is_throws_as) { //! OCLINT bitwise
                                                       //! operator in
                                                       //! conditional
            s << Color::Cyan << assertString(rb.m_at) << "( " << rb.m_expr
    ...
    void log_message(const MessageData& mb) override
    {
        ...
        s << getSuccessOrFailColor(false, mb.m_severity)
          << getSuccessOrFailString(
                 mb.m_severity & assertType::is_warn,
                 mb.m_severity,
                 "MESSAGE: ");
...
struct ConsoleReporterWithHelpers : public ConsoleReporter {
    ...
    {
        separator_to_stream();
        if (getContextOptions()->count
            || getContextOptions()->list_test_cases) {
            s << Color::Cyan << "[doctest] " << Color::None
...
#define DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(func, type) \
    ...
    DOCTEST_DEBUG_OUTPUT_WINDOW_REPORTER_OVERRIDE(
        test_run_start,
    ...
    int&               res)
{
    ...
    if (type == 0) {
        ...
        // uses undefined behavior - assumes '0' on unsuccessful parse...
...
#define DOCTEST_PARSE_STR_OPTION(name, sname, var, default)    \
    ...
    if (parseFlag(argc, argv, "dt-list-reporters")
        || parseFlag(argc, argv, "dt-lr")) {
    ...
    for (auto& curr : testArray) {
        ...
        // execute the test if it passes all the filtering
        {
            ...
            do {
                ...
                // call it from here only if we will continue looping for other
                // subcases and call it again outside of the loop for one final
                // time - with updated flags
                if (p->should_reenter == true) {
                    DOCTEST_ITERATE_THROUGH_REPORTERS(test_case_end, *g_cs);
                    // remove these flags - it is expected that the reporters
            ...
            bool ok_to_fail = (TestCaseFailureReason::ShouldHaveFailedAndDid
                               & p->failure_flags)
                              || (TestCaseFailureReason::CouldHaveFailedAndDid
                                  & p->failure_flags)
                              || (TestCaseFailureReason::FailedExactlyNumTimes
